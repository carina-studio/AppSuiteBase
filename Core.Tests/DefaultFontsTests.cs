...
class DefaultFontsTests : ApplicationBasedTests<MockAppSuiteApplication>
{
    ...
    /// </summary>
    /// <remarks>
    /// When the first CJK font doesn't cover the character, Avalonia 11.3 stops following the composite and resolves the
    /// character by an unordered scan of the whole font collection, which always lands on 'Noto Sans SC' here. The ordering
    /// of the remaining CJK fonts therefore has no effect, and neither does their presence in the composite at all. Upstream
    /// Avalonia PR #21435 (post-11.3) makes matching deterministic and culture-aware, so this test is expected to start
    /// failing on the upgrade which includes it. See the ordering caveat in Core/AGENTS.md.
    /// </remarks>
    [Test]
