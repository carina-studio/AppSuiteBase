...
#pragma warning restore SYSLIB1045
        ...
        void OnDragOverTabItem(object? sender, DragOnTabItemEventArgs e)
        {
            ...
            if (!e.DataTransfer.TryGetGCHandle(this.TabItemDataFormat, out var itemHandle) || itemHandle == default)
                return;
            var tabItem = itemHandle.Target as TabItem;
            if (tabItem is null)
                return;
        ...
        void OnDropOnTabItem(object? sender, DragOnTabItemEventArgs e)
        {
            ...
            if (!e.DataTransfer.TryGetGCHandle(this.TabItemDataFormat, out var itemHandle) || itemHandle == default)
                return;
            var item = itemHandle.Target as TabItem;
        ...
        void ShowTutorial(Visual anchor)
        {
            var tutorial = new Tutorial().Also(it =>
            {
                ...
                it.Bind(Tutorial.IconProperty, this.GetResourceObservable("Image/Icon.Lightbulb.Colored.Gradient"));
