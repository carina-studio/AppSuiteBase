...
- Prefer compiled bindings as much as possible. `x:CompileBindings="False"` is **not allowed** — it switches compiled bindings off for an entire scope, taking every binding in that scope down with the one that needed it. When a reflection binding genuinely is needed, opt that **single** binding out with the dedicated `{ReflectionBinding …}` markup extension instead. `{ReflectionBinding}` stays available for exactly those cases; use it deliberately, not as a default. `x:CompileBindings="True"` is redundant (compiled is the Avalonia 12 default) and is omitted.
...
- `ObservableProperty<T>` fields use the `XxxProp` suffix, never `XxxProperty`; `AvaloniaProperty` fields on controls keep `XxxProperty`.
