...
class ApplicationInfoDialogImpl : Dialog
{
	...
	public ApplicationInfoDialogImpl()
	{
		...
		this.SetValue(HasPrivacyPolicyProperty, this.Application.PrivacyPolicyVersion is not null && this.Application.PrivacyPolicy is not null);
		this.SetValue(HasUserAgreementProperty, this.Application.UserAgreementVersion is not null && this.Application.UserAgreement is not null);
		...
		this.updateScreenInfoAction = new(() =>
		{
			...
			if (Platform.IsMacOS)
			{
				...
				{
					var displayWidth = Native.MacOS.GetDisplayPixelWidth(new(windowBounds.X, windowBounds.Y, windowBounds.Width, windowBounds.Height));
					if (displayWidth > 0)
	...
	protected override void OnDataContextChanged(EventArgs e)
	{
		...
		if (this.DataContext is ApplicationInfo appInfo)
		{
			...
			// check change list
			this.SetValue(HasApplicationChangeListProperty, this.Application.ChangeList is not null);
	...
	{
		if (this.Application.PrivacyPolicy is not { } documentSource)
			return;
	...
	void ShowProductInfo(Panel view)
	{
		...
		// show name
		var dialogItem = (DialogItem)view.Children.First(it => it is DialogItem);
		if (!productManager.TryGetProductName(productId, out string? name))
			name = productId;
		if (dialogItem.Children[0] is Avalonia.Controls.TextBlock nameTextBlock)
